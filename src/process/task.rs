...
impl Task {
    ...
    #[inline]
    pub fn get_periodicity(&self) -> &Option<Either<Periodic, Aperiodic>> {
        &(self.metadata.0).0
    }
    ...
    #[inline]
    pub fn get_creation_time(&self) -> &RTCDateTime {
        &(self.metadata.1).2
    }
    ...
    #[inline]
    pub fn set_elapsed_running_time(&mut self, duration: Duration) {
        (self.metadata.1).1 = duration
    }

    #[inline]
    pub fn is_realtime(&self) -> bool {
        (self.metadata.0).0.is_some()
    }

    #[inline]
    pub fn is_periodic(&self) -> bool {
        self.is_realtime() && (self.metadata.0).0.as_ref().unwrap().is_left()
    }

    #[inline]
    pub fn is_aperiodic(&self) -> bool {
        self.is_realtime() && (self.metadata.0).0.as_ref().unwrap().is_right()
    }
}
    ...
    fn cmp(&self, other: &Self) -> Ordering {
        match (self.get_periodicity(), other.get_periodicity()) {
            (Some(periodicity_a), Some(periodicity_b)) => {
                ord_periodicity(&periodicity_a, &periodicity_b)
            }
            (Some(_), None) => Greater,
            (None, Some(_)) => Less,
            (None, None) => Equal,
